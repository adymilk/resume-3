## 个人简历

- 周理强 / 男 / 1995
- 工作经验：2年
- 技术博客：https://i66.me
- Github：[LeachZhou](https://github.com/LeachZhou)
- 期望职位：前端工程师
- 期望城市：上海
- 户籍所在地：安徽芜湖
- 邮箱：leachzhou@163.com
- QQ/微信：1178539345

## 技术清单
- 熟练使用HTML5、CSS3，熟悉Less、rem、flex等技术，对浏览器兼容问题、移动端适配问题有一定的经验和解决方案
- 熟练掌握JavaScript，熟悉ES5、ES6，熟练使用Vue/jQuery/Bootstrap框架，熟悉echart.js，amap.js，moment.js，day.js等类库，了解响应式布局和数据可视化
- 熟悉RestAPI后端接口处理、跨域处理方式，能够运用 Jsonp、cors、Proxy技术实现跨域请求数据
- 熟悉MVVM模式，熟悉vue-cli,vue-router,axios,vuex的使用
- 熟练使用MUI/APIcloud等Hybrid APP开发框架
- 掌握前端构建工具Gulp、webpack及Git等工具的使用
- 熟悉NPM包的管理
- 熟悉持续集成平台TravisC
- 熟悉MVC模式，能够运用PHP和ThinkPHP进行相关web开发
- 掌握sql语句，掌握操作MySQL等数据库工具
- 掌握前端网站性能优化，SEO优化

## 工作经历

### 北风网（2018年4月-至今）

#### 北风网移动端

[【预览】](http://m.ibeifeng.com) 

_项目介绍_：北风网移动端项目基本为推广页站点，总共有5个域名的站点，内容各不相同(只放出了一个预览)，在功能方面，主要进行来源信息采集及用户信息收集，用来进行SEM推广，暂无SEO要求。

_主要职责_：主要负责北风网移动端的改版、开发及维护

_技术栈_：

- vw（移动端适配方案）
- wow.js（特效解决方案）
- fastclick.js（取消移动端点击延迟方案）
- swiper.js（移动端滑动方案）

_项目页面_：

<img width="30%" src="https://user-images.githubusercontent.com/18715564/44453952-dfe68b00-a62c-11e8-9dab-ed264aaa7d1c.png">  <img width="30%" src="https://user-images.githubusercontent.com/18715564/44454130-466ba900-a62d-11e8-839e-506dd54ae03c.png">  <img width="30%" src="https://user-images.githubusercontent.com/18715564/44454205-7dda5580-a62d-11e8-880d-04b68566b8e5.png">

#### 北风网PC端

[【预览】](http://www.ibeifeng.com) 

_项目介绍_：北风网PC端项目总共有7个域名站点，公司新出课程或活动就会新增活动页，页面需要部署到多个站点，我们采用PHP后端引入渲染方案，成功做到活动页一改七改的效果。

_主要职责_：主要负责北风网PC端的部分改版及维护。

_项目页面_：

![](https://user-images.githubusercontent.com/18715564/44455097-cb57c200-a62f-11e8-9a58-959c6237880d.png)

![](https://user-images.githubusercontent.com/18715564/44454963-72882980-a62f-11e8-9fe6-e6bd01d8a65c.png)

### 上海睦诚商务有限公司（2017年5月-2018年4月）

#### 捷律出行

[【预览】](http://www.feixiangchuxing.com) 百度搜索【捷律出行】

_项目介绍_：捷律出行官网是一款在线约车网站，网站采用高德地图API接口amap.js来进行地图接口模块的开发，订单页面使用Vue.js来进行数据驱动，整个系统制作周期为五天。

_主要职责_：主要负责项目技术选型、页面开发、前后端对接、弹窗等组件封装。

<details><summary>主要功能：</summary><br>

- 预览登入
  - 登录
  - 注册
  - 忘记密码
- 在线约车
  - 点选地址获取信息
  - 模糊输入自动提示地址
  - 根据始终点车类自动计算价格
- 个人中心
  - 资料修改
  - 地址管理
  - 评价管理
  - 行程管理
  - 发票管理  
  - 钱包管理

</details>
<br>
_技术栈_：

- jquery 
- Vue (数据驱动)
- amap（地图接口解决方案）
- swiper（滑动切换方案）
- datetimepicker（时间选择方案）
- localStorage (本地存储方案)

_项目页面_：

![](https://user-images.githubusercontent.com/18715564/44456124-5e91f700-a632-11e8-91be-f247f1fcfc0d.png)

![](https://user-images.githubusercontent.com/18715564/44456609-8fbef700-a633-11e8-80ea-272b7e971702.png)


#### 索玛商城微信端及后台管理系统

[【预览】](http://suoma.181.021mc.net/) 微信公众号搜索【索玛suoma】或【suomapg】，关注公众号，点击VIP订货

_项目介绍_：

索玛移动端项目是一款商城购物系统，包括下单，支付，加入购物车，订单处理，优惠券，积分商品，售后评价，高级权限订单审核，订单导出等功能，移动端采用JQ weui作为UI，采用rem解决移动端适配，ThinkPHP作为后端数据渲染。

索玛后台管理系统是一个三级权限后台管理系统，对接索玛移动商城购物系统，包括数据图表化展示，商品管理，订单管理，退换货管理，广告管理，促销管理，评价管理，系统设置等功能。

_主要职责_：主要负责微信端项目技术选型、页面开发、前后端对接，后台管理端页面开发及援助后端开发。

<details><summary>微信端功能：</summary><br>


- 首页
  - 登录
  - 注册
  - 忘记密码
  - 推荐产品
  - 公告
- 订购
  - 三级分类
  - 搜索商品
  - 购物车管理
  - 收藏商品
  - 上拉加载更多
  - 订购商品
  - 结算（可多选结算）
  - 付款
- 我的
  - 资料管理
  - 订单管理
  - 物流管理
  - 物流管理
  - 评价管理
  - 通知查看
  - 优惠券管理
  - 积分商品兑换
  - 售后管理
  - 经理/督导权限订单审核
  - 经理/督导权限订单记录导出

</details>
<br>
_微信端页面_：

<img width="30%" src="https://user-images.githubusercontent.com/18715564/44500846-30a5c480-a6bd-11e8-8e4c-a80a8078d9ed.png"> <img width="30%" src="https://user-images.githubusercontent.com/18715564/44500863-4ca96600-a6bd-11e8-9563-0b04a2ae3bf5.png"> <img width="30%" src="https://user-images.githubusercontent.com/18715564/44501083-49fb4080-a6be-11e8-90bd-ea75eeb9b52c.png">

<details><summary>后台功能:</summary><br>


- 客户系统
  - 客户资料管理
  - 客户设置
  - 批量管理客户/门店
- 商品管理
  - 分类管理
  - 商品管理
  - 推荐商品管理
- 订单管理
  - 新订单/处理中/已发货/已完成/未付款/未月结/已月结
- 退换货管理
  - 未处理/已处理
- 广告管理
- 公告管理
- 促销管理
  - 积分兑换优惠券
  - 积分兑换商品
  - 兑换订单
- 评价管理
  - 新评论
  - 已审核
  - 未通过
- 系统设置
  - 短信配置
  - 邮箱配置
  - 物流接口配置
  - 权限配置

</details>
<br>
_后台页面_: (页面贴出均为演示数据)


![image](https://user-images.githubusercontent.com/18715564/44502076-68633b00-a6c2-11e8-8853-a0349991a572.png)

![image](https://user-images.githubusercontent.com/18715564/44501955-e70ba880-a6c1-11e8-882f-b7e1501a25de.png)

![image](https://user-images.githubusercontent.com/18715564/44501972-fb4fa580-a6c1-11e8-9b69-244d4c44cd2e.png)


#### 临沂大市场APP

~~[【预览】]()~~ 请在APPStore和应用宝自行搜索

_项目介绍_：

临沂大市场APP是一款广告投放的跨平台APP，采用APICloud开发框架进行开发，利用APICloud调用bMap、UIPullRefresh、citySelector等原生模块、利用Vue作为项目数据驱动，echo.js作为懒加载图片优化，包括推荐广告，分类，发布广告等功能。

_主要职责_：主要负责项目技术选型、APP开发、不同平台功能适配、APP打包签名上架热更新。

<details><summary>主要功能：</summary><br>

- 首页
  - 搜索
  - 帮助
  - 全部产品
  - 推荐产品
  - 天气查询
- 产品
  - 关注标签分类管理
  - 商品分权限查看
  - 联系商家订购
  - 关注商品
- 我的
  - 个人资料管理
  - 发布商品广告
  - 店铺商品管理
  - 店铺资料管理
  - 关注管理
  - 系统设置
    - 缓存管理
    - 更新APP
    - 省流量模式
    - 反馈
    - 等等...(我忘记了2333)


</details>
<br>

_APP页面_：

<img width="30%" src="https://user-images.githubusercontent.com/18715564/44502359-bf1d4480-a6c3-11e8-806e-b8c1ab0bf3f3.jpg"> 
<img width="30%" src="https://user-images.githubusercontent.com/18715564/44502361-bfb5db00-a6c3-11e8-937e-cfd771c3612e.jpg"> 
<img width="30%" src="https://user-images.githubusercontent.com/18715564/44502363-c0e70800-a6c3-11e8-9cc2-dc5a976784b1.jpg">

#### 爱摊位APP

_项目介绍_：爱摊位APP是一款多摊位用户发布租赁的跨平台APP，采用MUI开发框架进行开发，h5+调用原生组件，利用Vue作为项目数据驱动，融云接口作为聊天系统基础，调用GoogleMap作为地图接口基础，整个项目包括租赁摊位、发布摊位、评价、投诉、收藏、聊天、推送等功能。

_主要职责_：主要负责项目技术选型、APP开发、不同平台功能适配、APP打包签名上架。

<details><summary>主要功能：</summary><br>

- 开屏广告
- 登录/注册/忘记密码
- 用户守则及隐私权说明
- 首页
  - 搜索
  - Banner广告
  - 行业信息
  - 推荐产品
- 摊位
  - 摊位多条件筛选
  - 查看摊位信息
  - 查看摊位全部评价
  - 查看多个子摊位信息
  - 分权限显示摊主电话
  - 与摊主聊天
  - 加入购物车
  - 反馈投诉
  - 收藏摊位
- 聊天
  - 最近聊天
  - 好友管理
  - 搜索添加好友
  - 聊天推送
  - 好友记录管理
  - 好友聊天
  - 查询是否在线
  - 信息查询
  - 发送图片文字表情
- 我的
  - 个人资料管理
    - 昵称管理
    - 密码管理
    - 生日管理
    - 性别管理
    - 真实姓名管理
    - 电话管理
    - 地址管理
    - 邮箱管理
    - 银行卡管理
    - 身份证管理
    - 星级管理
    - 等级管理
    - 信用管理
  - 购物车
    - 批量摊位管理
    - 主摊位管理
    - 多子摊位管理
    - 摊位结算
  - 我的承租
    - 承租摊主信息推送
    - 已承租
    - 未付款
    - 退租
  - 我的摊位
    - 发布摊位
      - 模糊输入自动提示地址
      - 多日期选择
      - 多子摊位选择
      - 多信息设置
    - 摊位管理
      - 主摊位管理
      - 附带子摊位管理
      - 被承租管理
      - 被退租管理
      - 被承租信息推送
      - 被退租信息推送
      - 反馈退租信息推送
  - 我的收藏
    - 收藏管理
  - 我的钱包
    - 充值
    - 提现
    - 充值提现分种类记录
  - 我的评价
    - 给他人的评价
    - 来自租户的评价
  - 历史记录
    - 承租记录
      - 发布评价
    - 发布记录
  - 系统设置
    - 开屏广告
    - 缓存管理
    - 用户守则及隐私权查看
    - 退出登录
    - 等等

</details>
<br>
_APP页面_：

找不到了...Σ(っ°Д°;)っ


### 芜湖中菱机电设备有限公司（2016年7月-2017年5月）

_主要职责_：负责项目静态页面的开发，将设计图切图按照W3C标准制作成HTML页面，利用JavaScript及JQuery实现网站特效，且养成了良好的代码编写习惯。

## 开源项目

### :school_satchel: 一步一友

[【源码】](https://github.com/LeachZhou/lvyou) 


_项目介绍_：

项目采用Vue全家桶来开发单页面应用，UI采用了MuseUI，后端采用了Baas云服务Leancloud数据存储及RestAPI，在优化方面，采用了webpack对项目进行打包，在项目中使用路由懒加载，图片懒加载，keep-alive页面缓存来对项目进行性能优化。

_项目页面_：

<img width="30%" src="https://camo.githubusercontent.com/c1325f7776cf011d7bd8469b6167649bd845df45/687474703a2f2f696d672e626c6f672e6373646e2e6e65742f3230313730373132303832393132333034"> <img width="30%" src="https://camo.githubusercontent.com/dc96fa640124284afd05e05611239a0d6885a694/687474703a2f2f696d672e626c6f672e6373646e2e6e65742f3230313730373132303833303038323031"> <img width="30%" src="https://camo.githubusercontent.com/00522903988d01e1be3f84106586bcc65d4196ad/687474703a2f2f696d672e626c6f672e6373646e2e6e65742f3230313730373132303833303435323532">

### :black_nib: 小桥酒馆博客

[【预览】](https://leachzhou.github.io/blog) 
[【源码】](https://github.com/LeachZhou/blog)


_项目介绍_：

项目采用Vue-cli / axios / Less，页面UI由自己参考设计，后端采用GitHub API 进行数据交互，issues作为博客文章，release作为更新记录，projects作为功能档期，使用TravisCI作为持续集成，git push即可直接部署在GitHub托管页上。

_项目功能_：

- 昼夜主题切换 
- 导航滚动特效
- PV / UX 数据统计
- 博客文章 / md格式解析样式 / 分页 / 人性化时间 / 评论
- 动态切换第三方远程字体图标链接
- 摄影模块Grid布局
- 照片左右滑动预览PS前后特效
- 常用工具 / 免费线上观看VIP视频

_项目页面_：

![image](https://user-images.githubusercontent.com/18715564/44386676-c6bedb00-a555-11e8-9322-80e001436e0b.png)

### :world_map: Vue高德地图

[【预览】](https://leachzhou.github.io/vue-amap-demo) 
[【源码】](https://github.com/LeachZhou/vue-amap-demo)

_项目介绍_：

基于vue高德地图的一个操作demo，包括输入自动提示、点击获取地址存入输入框等功能。

_项目功能_：

- 输入模糊地址自动提示点选
- 点击地图自动定位中心点
- 点击地图自动获取地址名称/具体地址/地址经纬度

_项目页面_：

![image](https://user-images.githubusercontent.com/18715564/44387933-73e72280-a559-11e8-9687-d6c22c3199fc.png)

### :watch: 人性化时间原生JS插件

[【源码】](https://leachzhou.github.io/friendlytimejs) 
[【NPM】](https://www.npmjs.com/package/friendlytimejs) 

_项目介绍_：

原生JS插件，人性化时间/友好时间的插件，将一个时间字符串格式（yyyy-MM-dd HH:mm:ss）显示友好时间（如，1分钟内，2分钟前等等）。

## 自我评价

稍后补充...


## 毕业院校
大专 / 安徽电子信息职业技术学院

------------------------
_最后更新于：2017年08月21_